...
async function sendMokaOrder(cart, {
  ...
  await submitOrder({
    ...
    accept_order_notification_url:   `${NOTIF_BASE}/order-notify?event=accepted&order=${applicationOrderId}`,
    complete_order_notification_url: `${NOTIF_BASE}/order-notify?event=completed&order=${applicationOrderId}`,
    cancel_order_notification_url:   `${NOTIF_BASE}/order-notify?event=cancelled&order=${applicationOrderId}`,
    ...discountFields,
    order_items,
  });
}
