...
// ── Cart Row ──────────────────────────────────────────────────────────────────
const CartRow = React.memo(({ entry, onIncrement, onDecrement, onRemove, isUnavailable = false, isOpen = true }) => {
  ...
  return (
    <motion.div
      ...
      className={`flex items-start gap-3.5 py-4 border-b border-gray-100/80 last:border-0 ${isUnavailable ? "opacity-60" : ""}`}
    >
      {/* Gambar — grayscale kalau unavailable */}
      <div className="w-16 h-16 rounded-2xl overflow-hidden bg-gray-50 flex-shrink-0 relative">
        <img
        ...
        {isUnavailable && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 rounded-2xl">
            <span className="text-white text-[8px] font-black text-center leading-tight px-1">{unavailLabel}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {isUnavailable && (
            <span className="text-[10px] font-bold text-red-500 bg-red-50 border border-red-100 px-2 py-0.5 rounded-full flex-shrink-0">
              {unavailLabel}
            </span>
          )}
        </div>
        ...
        <p className={`font-bold text-sm mt-1.5 ${isUnavailable ? "text-gray-400 line-through" : "text-orange-500"}`}>
      ...
      <div className="flex flex-col items-end gap-2 flex-shrink-0">
        ...
        {/* Qty controls — disabled saat unavailable */}
        <div className={`flex items-center gap-1.5 rounded-full px-1.5 py-1 ${isUnavailable ? "bg-gray-50" : "bg-gray-100"}`}>
          <button
            onClick={() => !isUnavailable && (qty === 1 ? onRemove(entry.key) : onDecrement(entry.key))}
            disabled={isUnavailable}
            className="w-5 h-5 rounded-full bg-white text-gray-700 flex items-center justify-center shadow-sm hover:bg-gray-50 active:scale-90 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
          ...
          <motion.span key={qty} initial={{ scale: 1.4 }} animate={{ scale: 1 }}
            className={`font-bold text-xs w-3.5 text-center ${isUnavailable ? "text-gray-300" : "text-gray-800"}`}>
            {qty}
          </motion.span>
          <button
            onClick={() => !isUnavailable && onIncrement(entry.key)}
            disabled={isUnavailable}
            className="w-5 h-5 rounded-full bg-orange-500 text-white flex items-center justify-center shadow-sm hover:bg-orange-600 active:scale-90 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
          >
...
const CartPanel = ({
  ...
  const unavailableEntries = entries.filter(e =>
    !isOpen || (isItemUnavailable?.(e.itemId) ?? false)
  );
  ...
  <>
    ...
    {/* Warning banner jika ada item unavailable */}
    <AnimatePresence>
      {hasUnavailable && (
        <motion.div
        ...
        >
          <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-start gap-2.5">
            ...
            <div>
              <p className="text-red-600 text-xs font-bold leading-snug">
                {!isOpen
                  ? "Toko sedang tutup"
                  : `${unavailableEntries.length} item sedang habis`}
              </p>
              <p className="text-red-400 text-[11px] mt-0.5">
                Hapus item yang ditandai untuk melanjutkan checkout
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <AnimatePresence mode="popLayout">
          {entries.map((entry) => (
            <CartRow
              ...
              onRemove={onRemove}
              isUnavailable={!isOpen || (isItemUnavailable?.(entry.itemId) ?? false)}
              isOpen={isOpen}
            />
          ))}
        </AnimatePresence>
        ...
        <div className="px-6 pb-8 pt-2 flex-shrink-0 border-t border-gray-100">
          <div className="space-y-1 mb-4 text-sm">
            ...
            <div className="flex justify-between items-center">
              <span className="text-gray-400 flex items-center gap-1">Admin fee</span>
              <span className="font-semibold text-gray-500">+Rp{fmt(onlineFee)}</span>
            </div>
          ...
          <button
            ...
            className="w-full py-4 rounded-full text-white font-black text-[15px] tracking-tight transition-all hover:scale-[1.015] active:scale-[.98] shadow-lg shadow-orange-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:scale-100"
            style={{ background: "linear-gradient(135deg,#FF6B35,#e85d2a)" }}
          >
            {submitting ? "Memproses…" : hasUnavailable ? "Hapus item yang habis dulu" : "Lanjut ke Pembayaran →"}
          </button>
...
// ── Main CartSidebar ──────────────────────────────────────────────────────────
const CartSidebar = React.memo(({ cart, onClose, onIncrement, onDecrement, onRemove, onCheckout, submitting, isOpen = true, isItemUnavailable }) => {
  ...
  const panelProps = {
    ...
    submitting, isOpen, isItemUnavailable,
  };
