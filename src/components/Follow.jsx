      ...
      <div className={`grid grid-cols-3 gap-1 ${SPACING.container.padding}`}>
        {posts.map((post) => (
          ...
          >
            ...
            <div className={`absolute inset-0 bg-white opacity-0 md:group-hover:opacity-30 md:${TRANSITIONS.fast}`} />

            {/* Instagram icon on hover — desktop only */}
            <div className={`absolute inset-0 flex items-center justify-center opacity-0 md:group-hover:opacity-100 md:${TRANSITIONS.fast}`}>
              <div className={`bg-white/95 backdrop-blur-sm ${RADIUS.circle} p-3 sm:p-4 transform scale-0 md:group-hover:scale-100 md:${TRANSITIONS.fast}`}>
                <svg className="w-8 h-8 sm:w-10 sm:h-10 text-brand-navy" fill="currentColor" viewBox="0 0 24 24">
