...
const SocialMediaLinks = ({ className = "", size = 28 }) => {
  ...
  return (
    <div className={`flex gap-4 ${className}`}>
      {SOCIAL_MEDIA_CONFIG.map(({ id, name, url, icon: Icon }) => (
        <a
          key={id}
          href={url}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackSocialClick(id, url)}
          className={`text-brand-navy ${TRANSITIONS.hover.opacity} ${
            deviceType === "mobile" ? "active:opacity-70" : "hover:opacity-70"
          }`}
          aria-label={name}
        >
          <Icon size={size} />
    ...
    <div className={`${SPACING.container.maxWidth} mx-auto mt-20 mb-16`}>
      <div className={`flex lg:flex-row flex-col gap-4 ${SPACING.container.padding} lg:items-start items-center`}>
        ...
        <div className="lg:w-1/3 text-wrap">
          <h3 className={`mb-2 text-brand-navy ${TYPOGRAPHY.weight.bold}`}>Our Store</h3>
          <div className="flex gap-2 mb-2 items-center">
            ...
            <span className={`text-brand-navy ${TYPOGRAPHY.body.small}`}>
              CIMB Niaga Digital Lounge FEB UGM
            </span>
          </div>
          <div className="flex gap-2 mb-2 ml-8">
            <p className={`text-brand-navy ${TYPOGRAPHY.body.small}`}>
          ...
          >
            ...
            <span className={`text-brand-navy ${TYPOGRAPHY.body.small}`}>0851 1104 2497</span>
          </button>
        </div>

        <div className="lg:w-1/3 w-full">
          <h3 className={`mb-2 text-brand-navy ${TYPOGRAPHY.weight.bold}`}>Find Us At Maps</h3>
      ...
      <div
        className={`${SPACING.container.padding} mt-12 pb-12 flex lg:flex-row flex-col lg:space-y-0 lg:justify-between justify-start space-y-10 items-center`}
      >
        <span className={`text-brand-navy ${TYPOGRAPHY.body.small}`}>
